...
# points_df for random forests
library(party)
rf_points_df <- select(points_df, -game_id, -offense, -defense)
rf_fit <- cforest(points ~ ., data = rf_points_df, 
                  control = cforest_unbiased(ntree = 2500))
# vi <- varimp(rf_fit)
rf_pred <- predict(rf_fit, newdata = points_df)
points_rf_df <- points_df %>%
  mutate(rf_pred = rf_pred) %>%
  glimpse() %>%
  write_csv("data/points-rf.csv")
